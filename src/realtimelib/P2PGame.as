...
package realtimelib
{
	...
	public class P2PGame extends EventDispatcher implements IRealtimeGame
	{
		...
		public function connect(userName:String,userDetails:Object=null):void{
			_session = new P2PSession(serverAddr,groupName);			
		...
		public function sendStartGame():void{
			realtimeChannelManager.sendStream.send("receiveStartGame",_session.myUser.id);
		}
		/**
		 * send trigger to game over
		 */
		public function sendGameOver():void{
			realtimeChannelManager.sendStream.send("receiveGameOver",_session.myUser.id);
		}
		...
		public function sendPlayerReady():void{
			realtimeChannelManager.sendStream.send("receivePlayerReady",_session.myUser.id);
		}
		/**
		 * send trigger that player is out
		 */
		public function sendPlayerOut():void{
			realtimeChannelManager.sendStream.send("receivePlayerOut",_session.myUser.id);
		}
		...
		public function sendMousePositions(x:int,y:int):void{
			realtimeChannelManager.sendStream.send("receiveMousePositions",_session.myUser.id,x,y);
		}
		/**
		...
		public function sendMovement(direction:int, down:Boolean):void{
			realtimeChannelManager.sendStream.send("receiveMovement",_session.myUser.id,direction,down);
		}
		/**
		...
		public function sendPosition(position:Object):void{
			realtimeChannelManager.sendStream.send("receivePosition",_session.myUser.id,position);
		}
		...
		public function sendRotation(rotation:Number):void{
			realtimeChannelManager.sendStream.send("receiveRotation",_session.myUser.id,rotation);
		...
		public function sendSpeed(speed:Number):void{
			realtimeChannelManager.sendStream.send("receiveSpeed",_session.myUser.id,speed);
